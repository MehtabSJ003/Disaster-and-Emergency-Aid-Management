...
namespace sdds {
	...
	void AidMan::sort() {
		...
		for (int i = 0; i < m_iProductItems; i++)
		{
			for (int j = i + 1; j < m_iProductItems; j++) 
			{
				//Sorting based upon difference between quantity needed and quantity on hand
				if ((m_iProduct[i]->qtyNeeded()- m_iProduct[i]->qty()) < (m_iProduct[j]->qtyNeeded()- m_iProduct[j]->qty()))
				{
					swap= m_iProduct[i];
					m_iProduct[i] = m_iProduct[j];
					m_iProduct[j] = swap;
				}
			}
		}
	...
	int AidMan::shipItems(const char* filename, const Date& date) {
		//Opening the file
		...
		for (int i = 0; i < m_iProductItems; i++) 
		{
			//Checking if the quantity needed equals the quantity on hand
			if (m_iProduct[i]->qtyNeeded() == m_iProduct[i]->qty()) 
			{
				count++;
				//Setting the product display format to linear
				m_iProduct[i]->linear(true);
				file << "   " << count;
				file << " | " << *m_iProduct[i] << endl;
				//Removing the shipped product from the inventory
				remove(i);
			}
		}
		file << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
	...
	void AidMan::run() {
		...
		do
		{
			...
			case 6:
				...
				cout << "Shipping Order for " << shipItems("shippingOrder.txt", date) << " times saved" << endl;
				cout << endl;
				save();
				break;
			case 7:
